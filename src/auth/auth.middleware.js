...
exports.isAuth = async (req, res, next) => {
	...
	if (!accessTokenFromHeader) {
		return res.status(401).send(createResponse(1005, 'Vui lòng đăng nhập để sử dụng chức năng này!',{
	...
	if (!verified) {
		return res
			.status(401)
			.send(createResponse(1005, 'Bạn không có quyền truy cập vào tính năng này!',{}));
	}
